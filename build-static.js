...
async function buildStatic() {
  ...
  try {
    ...
    await copyRecursive(publicDir, outputDir);
  } catch (err) {
  ...
  try {
    await fs.access(buildPublicDir);
    // Copy build assets
    const copyRecursive = async (src, dest) => {
      await fs.mkdir(dest, { recursive: true });
      const entries = await fs.readdir(src, { withFileTypes: true });
    ...
    await copyRecursive(buildPublicDir, outputDir);
  } catch (err) {
    console.log('No build/client directory found');
  }
